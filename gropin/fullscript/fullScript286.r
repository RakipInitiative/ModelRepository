...
if(length(T)>1 & length(Phe)>1 & length(NaCl)>1) {
	par(mfrow = c(1,3))
	persp(T,Phe,z1,col = 'green',xlab='T',ylab='Phe',zlab='Sqr_mu_max',zlim=myZLim1,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
	persp(T,NaCl,z2,col = 'green',xlab='T',ylab='NaCl',zlab='Sqr_mu_max',zlim=myZLim2,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
	persp(Phe,NaCl,z3,col = 'green',xlab='Phe',ylab='NaCl',zlab='Sqr_mu_max',zlim=myZLim3,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
...
if(myZLim[1] == myZLim[2]) myZLim[2] <- myZLim[2]+1
	if(length(NaCl)==1) {
		persp(T,Phe,myZ,col = 'green',xlab='T',ylab='Phe',zlab='Sqr_mu_max',main=titleText,sub=paste('other variable: NaCl =',round(NaCl,digits = 2)),zlim=myZLim,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
	}
	if(length(Phe)==1) {
		persp(T,NaCl,myZ,col = 'green',xlab='T',ylab='NaCl',zlab='Sqr_mu_max',main=titleText,sub=paste('other variable: Phe =',round(Phe,digits = 2)),zlim=myZLim,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
	}
	if(length(T)==1) {
		persp(Phe,NaCl,myZ,col = 'green',xlab='Phe',ylab='NaCl',zlab='Sqr_mu_max',main=titleText,sub=paste('other variable: T =',round(T,digits = 2)),zlim=myZLim,theta=305,phi=20,shade=0.25,ticktype = 'detailed')
	}
}
#############################
