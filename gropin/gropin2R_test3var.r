...
#for(run in 1:nrModels){
  ...
  #adding all variables to parameter script
   for (j in 1:nrOfVariables) {
    ...
    myParScript <- append(myParScript,paste0(myVarNames[j],                   
                                             "_fixed <- ",
                                             round((myVarMin[j]+myVarMax[j])/2,digits=2)))
...
# implementing a sequence from min(Var) to max(Var)
  myModelScript <- append(myModelScript,"# variables of this model")
  for(var in 1:nrOfVariables) {
    myModelScript <- append(myModelScript,
                            paste0(myVarNames[var],
                                   " <- seq(",
                                   myVarNames[var],
                                   "_start, ",
                                   myVarNames[var],
  ...
  if(nrOfVariables==3) {
    myVisScript <- append(myVisScript,"visAxes <- c(visVar1,visVar2)")
    myVisScript <- append(myVisScript,
                          paste0("expectedAxes <- c('",
                          paste(myVarNames[1:nrOfVariables],collapse = "','"),
                          "')"))
    myVisScript <- append(myVisScript,"checkPresent <- match(expectedAxes,visAxes)")
    myVisScript <- append(myVisScript,"missingAxis <- expectedAxes[is.na(checkPresent)]")
    ...
    for(var in 1:nrOfVariables) {
      ...
      myVisScript <- append(myVisScript,"argumentsVisVar <- expand.grid(ax1,ax2,ax3)
...
secondChosenAxis <- expectedAxes[
  max(
    ...
    which(expectedAxes == expectedAxes[match(2,checkPresent)]))
      ...
      myVisScript <- append(myVisScript,"result <- matrix(unlist(response_surface(")
      myVisScript <- append(myVisScript,paste0(
          "\targumentsVisVar['",
          myVarNames[1:nrOfVariables],
          "']",collapse=","))
      myVisScript <- append(myVisScript,paste0(")),nrow=",
                                               lenOfVar,
                                               ",byrow=F)"))
      myVisScript <- append(myVisScript,"")
      myVisScript <- append(myVisScript,"#order of entries in result is always:
...
# this makes sure, the order of visVar1 and visVar2 are not relevant")
      ...
      myVisScript <- append(myVisScript,"rownames(result) <- unlist(variables[firstChosenAxis])
colnames(result) <- unlist(variables[secondChosenAxis])")
      
      myVisScript <- append(myVisScript,paste0("persp(as.numeric(rownames(result)),
...
\tzlab='",
as.character(growthModels$mumax[run]),
"',
