...
stdPortion <- 126.9
upperPortion <- 1000.0
logPtr <- c(2.24, 2.36, 2.37, 2.58, 2.82, 2.86, 3.16, 3.17, 3.47, 
            3.52, 3.57, 3.83, 3.83, 3.84, 3.87, 3.89, 3.89, 3.90, 
            3.94, 4.03, 4.09, 4.42, 4.53, 4.54, 4.54, 4.62, 4.62, 
            4.68, 4.73, 4.76,4.84, 4.92, 4.93, 4.95, 4.97, 5.20, 
            5.25, 5.27, 5.39, 5.47, 5.60, 5.83, 5.89, 5.95, 5.96, 
            6.02, 6.23, 6.38, 6.96, 7.37, 7.90, 8.20, 9.00, 9.00, 9.00)
CretLogMin <- -2.0
CretLogMax <- 7.0
CretLogStep <- 0.1
maxDistsShown <- 7
...
Cretlog <- runConsumerPhaseNauta$Cretlog # abscissa for doses
dose <- runConsumerPhaseNauta$dose # distribution of doses as a function of Cret
dosemean <- runConsumerPhaseNauta$dosemean # mean of all doses
doseMC <- runConsumerPhaseNauta$doseMC # distribution of doses given normal distribution of Cret
PrevExp <- runConsumerPhaseNauta$PrevExp # Prevalence of Exposure
#############################
...
# abscissa for visualisation
doseAbsLogMin <- 0
doseAbsLogStep <- 0.01
doseAbsLogMax <- 9
doseAbsLog <- seq(doseAbsLogMin,doseAbsLogMax,by=doseAbsLogStep)

# calculating CDF of doses given normal distribution of Cret ~ N(muCret, sigmaCret^2)
doseMCcdf <- ecdf(doseMC)
cumProbPprev <- (1-Pprev)+Pprev*doseMCcdf(10^(doseAbsLog))

# CDF plot
plot(doseAbsLog,cumProbPprev, type="l", lwd=3,xlab="log dose [cfu]",ylab = "cumulative probability", main="distribution of doses")

# additional information table plot
resultsCPM <- t(cbind(round(dosemean,0), round(PrevExp*100,1),Pprev,paste0("N{",muCret,", (",sigmaCret,")^2 }")))
